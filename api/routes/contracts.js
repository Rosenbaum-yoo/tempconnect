...
export function createContractsRouter(deps) {
  ...
  router.get("/contracts", requireAuth, rperm("contract.view"), async (req, res) => {
    // Org-Scoping: nur Contracts der eigenen Org anzeigen (als buyer oder supplier)
    const orgId = req.orgId;
    const items = await contractService.listContracts(pool, {
      buyer_org_id: req.query.buyer_org_id || orgId || null,
  ...
  router.post("/contracts", requireAuth, rperm("contract.create"), async (req, res) => {
    ...
    try {
      ...
      logger.error({ err: err.message }, "Contract create failed");
      res.status(500).json({ error: "SERVER_ERROR" });
    }
  });

  router.get("/contracts/:id", requireAuth, rperm("contract.view"), async (req, res) => {
  ...
  router.patch("/contracts/:id", requireAuth, rperm("contract.edit"), async (req, res) => {
  ...
  router.post("/contracts/:id/activate", requireAuth, rperm("contract.create"), async (req, res) => {
    const existing = await contractService.getContract(pool, req.params.id);
    if (!existing) return res.status(404).json({ error: "NOT_FOUND" });
    if (req.orgId && existing.buyer_org_id !== req.orgId) {
      return res.status(403).json({ error: "ORG_BOUNDARY_VIOLATION" });
  ...
  router.post("/contracts/:id/terminate", requireAuth, rperm("contract.terminate"), async (req, res) => {
    const existing = await contractService.getContract(pool, req.params.id);
    if (!existing) return res.status(404).json({ error: "NOT_FOUND" });
    if (req.orgId && existing.buyer_org_id !== req.orgId) {
      return res.status(403).json({ error: "ORG_BOUNDARY_VIOLATION" });
