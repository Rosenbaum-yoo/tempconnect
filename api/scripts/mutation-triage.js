...
export function werteAus(ergebnis = lade(ERGEBNIS_PFAD), triage = lade(TRIAGE_PFAD)) {
  ...
  return {
    ...
    summe: dateien.reduce(
      ...
      { gesamt: 0, A: 0, B: 0, C: 0, A_hoch: 0, zweig: 0, art_logik: 0 }
    ),
...
function main() {
  ...
  console.log(
    tabelle([
      ["Datei", "Score", "Ueberlebt", "A", "davon hoch", "B", "C", "Art=Logik", "im Zweig", "Zweig offen"],
      ...a.dateien.map((d) => [
        d.datei,
  ...
  console.log(
    "\n  Art=Logik  = mechanisch: Conditional/Logical (so entstand die Zahl 28 im Bericht)" +
      "\n  im Zweig   = beurteilt: sitzt in einer Verzweigung, die Verhalten steuert" +
      "\n  Zweig offen= davon Kategorie A — das ist der Rest, den das Gate der Direktive meint"
  );
