...
function slaBadge(respondBy, status) {
  if (status === "BREACHED") return "<span class=\"tag red\">Pulse überschritten</span>";
  if (!respondBy) return "<span class=\"tag muted\">–</span>";
  var min = Math.floor((new Date(respondBy).getTime() - Date.now()) / 60000);
  if (min <= 0) return "<span class=\"tag red\">Abgelaufen</span>";
  if (min <= 15) return "<span class=\"tag yellow timer\">" + min + " min</span>";
  return "<span class=\"tag green timer\">" + min + " min</span>";
}
...
function load() {
  api("/my/requests/received").then(function(r) {
    ...
    function compBadge(s) {
      if (!s) return "";
      var c = (s + "").toUpperCase(), cls = c === "GREEN" ? "green" : c === "YELLOW" ? "yellow" : "red";
    ...
    data.forEach(function(r) {
      ...
      html += "<div>" + comp + sla + " <strong>" + role + "</strong> von " + from + " <span class=\"tag muted\">" + r.status + "</span></div>";
      html += "<div style=\"display:flex;gap:8px\">";
      html += "<a href=\"/public/request_detail.html?id=" + r.id + "\" class=\"btn\">Details</a>";
      if (r.status === "SENT" || r.status === "OFFERED") {
        html += "<button type=\"button\" class=\"btn good\" data-id=\"" + r.id + "\" data-action=\"ACCEPTED\">Annehmen</button>";
        html += "<button type=\"button\" class=\"btn bad\" data-id=\"" + r.id + "\" data-action=\"DECLINED\">Ablehnen</button>";
      }
      html += "</div></div></div>";
    });
    list.innerHTML = html;
