...
Der kanonische Artefaktbau läuft über den CI-Job `release-artifact` in `.github/workflows/ci.yml`.
...
- fehlende Pflichtdateien wie `docker-compose.prod.yml`, `scripts/prod-update.sh`, `scripts/prod-up.sh`, Backup-/Restore-Skripte oder `README.md`

Das Artefakt enthält ein Manifest mit Quelle (`git-archive`), Git-Ref und SHA-256.
